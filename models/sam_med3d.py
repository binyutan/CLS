...
Wang et al., arXiv:2310.15161, 2023.

Replace the placeholder model with the actual SAM-Med3D architecture
from the official repository: https://github.com/uni-medical/SAM-Med3D
...
class SAMMed3DSegNet(nn.Module):
    """Placeholder — replace with actual SAM-Med3D architecture."""
    def __init__(self, n_seg_classes=2):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Conv3d(1, 64, 3, padding=1), nn.LayerNorm([64, 1, 1, 1]), nn.GELU(),
            nn.Conv3d(64, 128, 3, padding=1), nn.LayerNorm([128, 1, 1, 1]), nn.GELU(),
        )
...
def generate_model(args):
    model = SAMMed3DSegNet(n_seg_classes=args.n_seg_classes)
    if not args.no_cuda:
        model = model.cuda()
